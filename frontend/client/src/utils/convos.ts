...
import { addData, deleteData, findPage, updateData } from "./collection";
...
export const groupConversationsByDate = (
  ...
  const yearMonthGroups = Array.from(groups.keys())
    ...
    .sort((a, b) => {
      const [yearA, yearB] = [
        Number.parseInt(a.trim()),
        Number.parseInt(b.trim()),
      ];
      if (yearA !== yearB) {
        return yearB - yearA;
      }
...
export const updateConvoFields = (
  data: ConversationData,
  updatedConversation: Partial<TConversation> &
  ...
  if (pageIndex !== -1 && index !== -1) {
    const oldConversation = newData.pages[pageIndex].conversations[
      index
    ] as TConversation;
...
export const deleteConversation = (
  ...
  return deleteData<ConversationListResponse, ConversationData>(
    data,
    "conversations",
    (page) =>
