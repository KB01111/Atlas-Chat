...
function useTextToSpeechEdge({
  ...
  const fetchVoices = useCallback(() => {
    ...
    ttsRef.current
      ...
      .catch((error) => {
        ...
        showToast(localize('com_nav_voices_fetch_error'));
      });
  }, [showToast, localize]);
    ...
    if (availableVoice) {
      ...
      ttsRef.current
        ...
        .catch((error) => {
          ...
          showToast(localize('com_nav_tts_init_error', { 0: (error as Error).message }));
        });
    ...
    if (
      ...
      if (nextBuffer) {
        try {
          ...
          showToast(localize('com_nav_buffer_append_error'));
  ...
  const onSourceOpen = useCallback(() => {
    if (!sourceBufferRef.current && mediaSourceRef.current) {
      try {
        ...
        showToast(localize('com_nav_source_buffer_error'));
      }
    }
    ...
    (text: string) => {
      const generate = async () => {
        try {
          ...
          showToast(localize('com_nav_audio_play_error', { 0: (error as Error).message }));
          setIsSpeaking(false);
        }
      };
  ...
  const cancelSpeechEdge = useCallback(() => {
    try {
      ...
      showToast(localize('com_nav_speech_cancel_error'));
    }
