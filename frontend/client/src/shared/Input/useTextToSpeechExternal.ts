...
function useTextToSpeechExternal({
  ...
  const playAudioPromise = (blobUrl: string) => {
    ...
    playPromise().catch((error: Error) => {
      ...
      console.error(error);
      showToast(localize('com_nav_audio_play_error', { 0: error.message }));
    });
  ...
  const { mutate: processAudio, isLoading } = useTextToSpeechMutation({
    onMutate: (variables) => {
      ...
      if (inputText.length >= 4096) {
        showToast(localize('com_nav_long_audio_warning'));
      }
    },
    onSuccess: async (data: ArrayBuffer, variables) => {
      try {
        ...
        showToast(`Error processing audio: ${(error as Error).message}`);
      }
    },
    onError: (error: unknown) => {
      showToast(localize('com_nav_audio_process_error', { 0: (error as Error).message }));
    },
  });
