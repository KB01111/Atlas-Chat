...
const useSpeechToTextExternal = (
  ...
  const { mutate: processAudio, isLoading: isProcessing } = useSpeechToTextMutation({
    ...
    onError: () => {
      showToast('An error occurred while processing the audio, maybe the audio was too short');
      setIsRequestBeingMade(false);
    },
  });
  ...
  const handleStop = () => {
    if (audioChunks.length > 0) {
      ...
      showToast('The audio was too short');
    }
  };
  ...
  const startRecording = async () => {
    if (isRequestBeingMade) {
      showToast('A request is already being made. Please wait.');
      return;
    }
  ...
  const stopRecording = () => {
    ...
    if (mediaRecorderRef.current.state === 'recording') {
      ...
      showToast('MediaRecorder is not recording');
    }
  };

  const externalStartRecording = () => {
    if (isListening) {
      showToast('Already listening. Please stop recording first.');
      return;
    }
  ...
  const externalStopRecording = () => {
    if (!isListening) {
      showToast('Not currently recording. Please start recording first.');
      return;
    }
