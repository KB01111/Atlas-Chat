...
describe("convertJsonSchemaToZod", () => {
  ...
  describe("schema descriptions", () => {
    ...
    it("should preserve descriptions in nested objects", () => {
      ...
      if ("user" in shape) {
        ...
        const userShape =
          shape.user instanceof z.ZodObject ? shape.user.shape : {};
        if ("name" in userShape && "settings" in userShape) {
          ...
          const settingsShape =
            userShape.settings instanceof z.ZodObject
              ? userShape.settings.shape
              : {};
    ...
    it("should preserve descriptions in a complex schema", () => {
      ...
      if ("basicInfo" in shape) {
        ...
        const basicInfoShape =
          shape.basicInfo instanceof z.ZodObject ? shape.basicInfo.shape : {};
      ...
      if ("preferences" in shape) {
        ...
        const preferencesShape =
          shape.preferences instanceof z.ZodObject
            ? shape.preferences.shape
            : {};

        if (
          "notifications" in preferencesShape &&
          "theme" in preferencesShape
        ) {
          expect(preferencesShape.notifications.description).toBe(
            "Notification settings",
          );
          expect(preferencesShape.theme.description).toBe(
            "UI theme preference",
          );
        }
      }
  ...
  describe("additionalProperties handling", () => {
    it("should allow any additional properties when additionalProperties is true", () => {
      ...
      expect(
        zodSchema?.parse({ name: "John", tags: ["tag1", "tag2"] }),
      ).toEqual({
        name: "John",
        tags: ["tag1", "tag2"],
      });
    ...
    it("should validate additional properties according to schema when additionalProperties is an object", () => {
      ...
      expect(() =>
        zodSchema?.parse({ name: "John", isActive: true }),
      ).toThrow();
      expect(() =>
        zodSchema?.parse({ name: "John", tags: ["tag1", "tag2"] }),
      ).toThrow();
    ...
    it("should strip additional properties when additionalProperties is false or not specified", () => {
      ...
      const zodSchemaWithoutAdditionalProps = convertJsonSchemaToZod(
        schemaWithoutAdditionalProps,
      );

      expect(
        zodSchemaWithoutAdditionalProps?.parse({ name: "John", age: 30 }),
      ).toEqual({
        name: "John",
        age: 30,
      });
  ...
  describe("empty object handling", () => {
    it("should return undefined for empty object schemas when allowEmptyObject is false", () => {
      ...
      emptyObjectSchemas.forEach((schema) => {
        expect(
          convertJsonSchemaToZod(schema, { allowEmptyObject: false }),
        ).toBeUndefined();
      });
    });
    ...
    it("should still convert non-empty object schemas regardless of allowEmptyObject setting", () => {
      ...
      const resultWithFlag = convertJsonSchemaToZod(schema, {
