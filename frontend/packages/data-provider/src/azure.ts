...
export function validateAzureGroups(
  configs: TAzureGroups,
): TAzureConfigValidationResult {
  let isValid = true;
  ...
  if (!result.success) {
    ...
    for (const group of result.data) {
      ...
      if (groupMap[groupName]) {
        errors.push(
          `Duplicate group name detected: "${groupName}". Group names must be unique.`,
        );
      ...
      if (serverless && !baseURL) {
        errors.push(
          `Group "${groupName}" is serverless but missing mandatory "baseURL."`,
        );
        return { isValid: false, modelNames, modelGroupMap, groupMap, errors };
      }
      ...
      for (const modelName in group.models) {
        ...
        if (typeof model === "boolean") {
          ...
          if (
            (!modelDeploymentName && !groupDeploymentName) ||
            (!modelVersion && !groupVersion)
          ) {
            errors.push(
...
export function mapModelToAzureConfig({
  ...
  if (groupConfig.serverless === true) {
    ...
    if (typeof apiKeyValue === "string" && envVarRegex.test(apiKeyValue)) {
      throw new Error(
        `Azure configuration environment variable "${apiKeyValue}" was not found.`,
      );
    }
  ...
  for (const value of Object.values(azureOptions)) {
    if (typeof value === "string" && envVarRegex.test(value)) {
      throw new Error(
        `Azure configuration environment variable "${value}" was not found.`,
      );
    }
  }
...
export function mapGroupToAzureConfig({
  ...
  if (models.length === 0) {
    throw new Error(
      `Group "${groupName}" does not have any models configured.`,
    );
  }
  ...
  azureOptions.azureOpenAIApiDeploymentName =
    extractEnvVariable(deploymentName);
