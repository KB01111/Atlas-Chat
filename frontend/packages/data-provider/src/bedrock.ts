...
export const bedrockInputSchema = s.tConversationSchema
  ...
  .transform((obj) => {
    if (
      (obj as AnthropicInput).additionalModelRequestFields?.thinking != null
    ) {
      ...
      obj.thinkingBudget =
        typeof _obj.additionalModelRequestFields.thinking === "object"
          ? (_obj.additionalModelRequestFields.thinking as ThinkingConfig)
              ?.budget_tokens
          : undefined;
...
export const bedrockInputParser = s.tConversationSchema
  ...
  .transform((data) => {
    ...
    if (
      ...
      if (
        additionalFields.thinking === true &&
        additionalFields.thinkingBudget === undefined
      ) {
      ...
      additionalFields.thinkingBudget != null
    ) {
      delete additionalFields.thinking;
      delete additionalFields.thinkingBudget;
    }
    ...
    if (Object.keys(additionalFields).length > 0) {
      typedData.additionalModelRequestFields = {
        ...((typedData.additionalModelRequestFields as
...
function configureThinking(data: AnthropicInput): AnthropicInput {
  ...
  if (updatedData.additionalModelRequestFields?.thinking === true) {
    updatedData.maxTokens =
      updatedData.maxTokens ?? updatedData.maxOutputTokens ?? 8192;
    delete updatedData.maxOutputTokens;
    const thinkingConfig: AnthropicReasoning["thinking"] = {
      type: "enabled",
      budget_tokens:
        updatedData.additionalModelRequestFields.thinkingBudget ?? 2000,
    };
...
export const bedrockOutputParser = (data: Record<string, unknown>) => {
  const knownKeys = [
    ...Object.keys(s.tConversationSchema.shape),
    "topK",
    "top_k",
  ];
  ...
  if (
    ...
    Object.entries(
      data.additionalModelRequestFields as Record<string, unknown>,
    ).forEach(([key, value]) => {
      if (knownKeys.includes(key)) {
        if (key === "top_k") {
          result["topK"] = value;
  ...
  if (
    (result as AnthropicInput).additionalModelRequestFields?.thinking == null
  ) {
