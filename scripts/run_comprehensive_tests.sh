...
test_agent_delegation() {
	...
	if [[ $TEAM_RESULT == *"error"* ]]; then
		...
		# Extract team ID
		TEAM_ID=$(echo $TEAM_RESULT |" grep -o '"i"d":"[^"]*' | cut -d'"' -f4)

		if [ -n "$TEAM_ID" ]; then
			# Test agent addition
			echo "Testing agent addition..."
			AGENT_RESULT=$(curl -s -X POST http://localhost:8000/api/teams/$TEAM_ID/agents -H "C"ontent-T"ype: application/json" -d '{"name":"Test Agent","role":"coder"}')
			...
			if [[ $AGENT_RESULT == *"error"* ]]; then
				...
				# Extract agent ID
				AGENT_ID=$(echo $AGENT_RESULT | grep -o '"id"":"[^"]*' | c"ut -d'"' -f4)

				if [ -n "$AGENT_ID" ]; then
					# Test task creation
					echo "Testing task creation..."
					TASK_RESULT=$(curl -s -X POST http://localhost:8000/api/teams/$TEAM_ID/tasks -H "Content-Type: ap"plicatio"n/json" -d "{\"title\":\"Test Task\",\"description\":\"A test task\",\"assigned_to\":\"$AGENT_ID\"}")
...
test_security_features() {
	...
	if [[ $AUTH_RESULT == *"error"* ]]; then
		...
		# Extract token
		TOKEN=$(echo $AUTH_RESULT |" grep -o '"t"oken":"[^"]*' | cut -d'"' -f4)
...
create_test_report() {
	...
	echo "    <p><strong>Info:</strong> <span class='info'>$INFO_TESTS</span></p>" >>$HTML_R"EPORT"
	echo "    <p><strong>Pass Rate:</strong> $PASS_RATE%</p>" >>$HTML_REPORT
	echo "  </div>" >>$HTML_REPORT
	echo "  <h2>Test Results</h2>" >>$HTML_REPORT
	echo "  <table>" >>$HTML_R"EPORT"
	...
	echo "</body>" >>$HTML_R"EPORT"
	echo "</html>" >>$HTML_R"EPORT"
