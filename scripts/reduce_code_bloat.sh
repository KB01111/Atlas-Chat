...
cleanup_frontend() {
	...
	find $FRONTEND_DIR/styles -type f -name "*.css" -o -name "*.scss" | while read file; do
		...
		if ! grep -q "$(basename $file)" $FRONT"""END"_"D"IR --include="*.js" --include="*.jsx" --include="*.ts" --include="*.tsx"; then
	...
	find $FRONTEND_DIR/utils -type f -name "*.js" -o -name "*.ts" | while read file; do
		...
		if ! grep -q "$(basename $file | cut -d""". -"f"1")" $FRONTEND_DIR --include="*.js" --include="*.jsx" --include="*.ts" --include="*.tsx"; then
...
cleanup_backend() {
	...
	find $BACKEND_DIR -type f -name "*.py" | while read file; do
		...
		if [[ "$(basename $file)" == "__"""ini"t"_"_.py" ]]; then
			skip=true
		fi
