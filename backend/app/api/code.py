...
async def execute_code(
    ...
    result = await tool_executor.execute_code(
        code=request.code, language=request.language, context=context
    )
...
async def write_file(
    ...
    result = await tool_executor.write_file(
        file_path=request.file_path, content=request.content, context=context
    )
...
async def read_file(
    ...
    # Read file
    result = await tool_executor.read_file(file_path=request.file_path, context=context)
...
async def install_packages(
    ...
    result = await tool_executor.install_packages(
        packages=request.packages, language=request.language, context=context
    )
