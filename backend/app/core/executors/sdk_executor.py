...
class SDKExecutor:
    ...
    async def _stream_run_response(self, thread_id: str, run_id: str) -> AsyncGenerator[str, None]:
        ...
        while True:
            ...
            if run.status == "completed":
                ...
                # Handle tool calls
                await self._process_tool_calls(thread_id, run_id, run.required_action.submit_tool_outputs.tool_calls)
    ...
    async def _execute_tool(self, function_name: str, args: Dict[str, Any]) -> Any:
        ...
        if function_name == "execute_code":
            ...
            return await self.tool_executor.list_files(
                args.get("path", "./"),
                None
            )
            ...
            return await self.tool_executor.add_graphiti_episode(
                args.get("episode_text", ""),
                None,
                args.get("name", "interaction")
            )
        
        elif function_name == "search_graphiti":
            return await self.tool_executor.search_graphiti(
                args.get("query", ""),
                None
            )
            ...
            return await self.tool_executor.web_search(
                args.get("query", ""),
                None
            )
