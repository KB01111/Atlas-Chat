...
def test_execute_code_error(mock_tool_executor):
    ...
    # Check response
    assert response.status_code == 200
    assert response.json() == {"success": False, "error": "Syntax error"}


def test_write_file_success(mock_tool_executor):
    # Mock the write_file method to return a successful result
    mock_tool_executor.write_file.return_value = {"success": True}
...
def test_read_file_success(mock_tool_executor):
    ...
    # Check response
    assert response.status_code == 200
    assert response.json() == {"success": True, "content": "print('Hello, World!')"}
...
def test_unauthorized_access():
    ...
    response = client.post("/code/execute", json=test_data)
