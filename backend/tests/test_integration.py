...
# Mock test user
TEST_USER = {"id": "test-user-id", "name": "Test User", "email": "test@example.com"}
...
def test_login_endpoint(mock_db_session):
    ...
    response = client.post(
        "/api/auth/login", data={"username": TEST_USER["email"], "password": "password"}
    )
    assert response.status_code == 200
...
def test_chat_endpoint(mock_db_session):
    ...
    with patch("app.api.chat.AgentService") as MockAgentService:
        ...
        response = client.post(
            "/api/chat",
            headers={"Authorization": f"Bearer {token}"},
            json={"agent_id": "agent-1", "message": "Hello, AI!", "history": []},
            stream=True,
        )
        assert response.status_code == 200
