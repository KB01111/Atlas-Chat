...
try:
    ...
    from app.core.services.agent_factory.agent_definition import AgentDefinition, AgentRequest, AgentMessage
    from app.core.services.agent_factory.agent_specialization import SpecializationRegistry
    from app.core.services.agent_factory.agent_team import TeamRegistry, AgentTeam, TeamMember
        ...
        try:
            ...
            if model_spec:
                ...
                if provider.lower() == case["expected_provider"].lower():
                    ...
                    logger.warning(f"❌ Test failed: Expected provider {case['expected_provider']}, got {provider}")
            else:
...
def test_agent_factory():
    ...
    total_tests = len(test_agents) + 4  # Creation tests + update + deletion + specialization + team + memory
